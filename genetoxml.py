...
def getProgramPath(arg):
    ...
    if arg is not None:
        ...
        if not os.path.exists(gene2xml):
            return None
            #print_verbose(0, '[Error] The program path %s does not exist.' % gene2xml)
            #return
    else:
...
def convertGenetoXml(program, infile): 
    # the outfile has the same file name with the input file but having xml extension.
    extension = infile.rfind('.ags')
    if extension == -1:  # file is not ags format
        print_verbose(0, '[Error] The file to be converted is not ags file (%s).' % infile)
        return
    ...
    try:
        ...
        if e.errno == 2:  # linux.gene2xml is not in PATH
            print_verbose(0, '[Error] Could not execute the program linux.gene2xml, perhaps it is not installed in your PATH? Please add the program path to PATH or use --gene2xmlpath option.')
        else:
            print_verbose(0, 'Undefined error occurs while converting data to xml.')
            print_verbose(0, '  [Error Log] ' + str(e.errno))
    else:
        print_verbose(2, '  >> Converting %s to %s is successful.' % (infile, outfile))
    finally:
        return outfile


# Ftp class supports connect(), disconnect(), download(), getLastModifiedTime(), and getSize().
...
class Ftp(object):
    ...
    def getLastModifiedTime(self, filepath):
        ...
        try:
            ...
            print_verbose(2, '  >> Getting the last modified time of data is successful.')
            print_verbose(2, '  >> The last modified time of the data at the server is %s' % str(mdServer))
    ...
    def getSize(self, filepath):
        ...
        try:
            ...
            print_verbose(2, '  >> The file size is %.2fM' % (dataSize/(1024**2)))
        except ftplib.all_errors as e:  
...
def main():
    ...
    if args.downloadonly and args.convertonly:
        print_verbose(0, '[Error] Either --downloadonly or --convertonly can be used at once')
        return
    # check the linux.gene2xml path
    gene2xml = getProgramPath(args.gene2xmlpath)
    if gene2xml is None:
        print_verbose(0, '[Error] The program path %s does not exist.' % args.gene2xmlpath)
        return
    ...
    if args.convertonly is not None:
        ...
        # download the file
        if uptodatefile == False or args.force:
            storefile = ftp.download(dataFilePath, mdServer)
            if storefile is None:
                ftp.disconnect()
                return
        else: 
            print_verbose(0, 'You already have the latest version of the file (%s_%s%s).' % (filePrefix, mdServer, fileExt))
            ftp.disconnect()
            return
